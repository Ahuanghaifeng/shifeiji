...
import com.sfj.sfj.utils.FileUtil;
...
public class AppException extends Exception implements UncaughtExceptionHandler {
    ...
    private AppException(Context context) {
        this.mContext = context;//(AppContext) context;
...
//            return http(e);
    ...
    private boolean saveToSDCard(Throwable ex) throws Exception {
        boolean append = true;
        if (FileUtil.checkFilePathExists(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "91Y/log")) {
            List<File> list = FileUtil.listPathFiles(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "91Y/log");
        ...
        File file = FileUtil.getSaveFile("91Y/log", getDataTime("MM_dd_HH_mm_ss") + ".txt");
        PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(
                file, append)));
        // 导出发生异常的时间
        pw.println(getDataTime("yyyy_MM_dd_HH_mm_ss"));
        // 导出手机信息
        dumpPhoneInfo(pw);
        pw.println();
        // 导出异常的调用栈信息
